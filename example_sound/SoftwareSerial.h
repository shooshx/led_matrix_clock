...
from https://github.com/junhuanchen/Esp32-SoftwareSerial/tree/master
*/
...
class SoftwareSerial : public Stream
{
  public:
    ...
    void begin(uint32_t speed, int receivePin, int transmitPin, bool inverse_logic = false, unsigned int buffSize = 64);
    long baudRate();
...
void SoftwareSerial::begin(uint32_t speed, int receivePin, int transmitPin, bool inverse_logic, unsigned int buffSize)
{
  ...
  if (isValidGPIOpin(receivePin))
  {
    ...
    if (m_buffer != NULL)
    {
	  ...
	  Serial.printf("Invalid rx pin! %d\n", receivePin);
  }
  ...
  if (isValidGPIOpin(transmitPin))
  {
    ...
    m_txValid = true;
	  ...
	  Serial.printf("Invalid tx pin! %d\n", transmitPin);
  }
