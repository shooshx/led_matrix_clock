...
function create_at(elem, add_func, sz, visible, onchange, options, start_color) 
{
    ...
    const touch_to_mouse = function(e) {
        const t = e.touches[0]
        return { clientX: t.clientX, clientY: t.clientY, buttons: 1, target:e.target }
    }
    ...
    const touch_move = function(e) {
        e.preventDefault()
        mouse_act(touch_to_mouse(e))
    }
    const doc_mouseup =  function() {
    ...
    options.myAddEventListener(document, "mouseup", doc_mouseup)
    
    const canv_mousedown = function(e) {
        let do_capture = mouse_act(e, true)
        if (do_capture) {
            options.myAddEventListener(document, "mousemove", mouse_move)
            options.myAddEventListener(document, "touchmove", touch_move)
        }                
    }
    options.myAddEventListener(canvas, "mousedown", canv_mousedown)
    ...
    options.myAddEventListener(canvas, "touchstart", (e)=>{
        //console.log('cc-start')
        e.preventDefault()
        canv_mousedown(touch_to_mouse(e))
    })
    options.myAddEventListener(canvas, "touchend", (e)=>{
        //console.log('cc-end')
        e.preventDefault()
        doc_mouseup()
    })
    options.myAddEventListener(canvas, "touchcancel", (e)=>{
        //console.log('cc-cancel')
        e.preventDefault()
        doc_mouseup(touch_to_mouse(e))
    });
...
function make_overlay(edit_elem)
{
    const back_overlay = document.createElement('div')
    back_overlay.style.backgroundColor = 'transparent'
    back_overlay.style.position = 'fixed'
    back_overlay.style.top = 0
    back_overlay.style.left = 0
    back_overlay.style.right = 0
    back_overlay.style.bottom = 0
    back_overlay.style.display = 'none'
    edit_elem.parentNode.insertBefore(back_overlay, edit_elem.nextSibling)
    const set_visible = function(v) {
        back_overlay.style.display = v ? 'initial' : 'none'
    }
    return { elem:back_overlay, set_visible: set_visible}
}
...
{
    const overlay = make_overlay(edit_elem)
    const picker = ColorPicker.create_after(edit_elem, sz, false, function(c, trigger_level2=true) { 
    ...
    if (!DEBUG_NO_BLUR) {
        options.myAddEventListener(overlay.elem, "click", function(e) {
